...
#define WIFI_CH        1
#define MAX_PAYLOAD    180        // ✅ must match sender
#define MAX_JPEG       120000
#define SWAP_BYTES     true

// Buttons
#define BTN_SELECT     0   // Boot button
#define BTN_CHANGE     35

TFT_eSPI tft = TFT_eSPI();

// Placeholder for Sender MAC - UPDATED MANUALLY BY USER
uint8_t senderMac[] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF}; 

enum : uint8_t { PKT_START = 1, PKT_DATA = 2, PKT_CONFIG = 3 };
...
void drawUI() {
  // Simple overlay or separate status bar
  // Let's use top 20 pixels for status
  tft.fillRect(0, 0, 320, 20, TFT_NAVY);
  tft.setTextColor(TFT_WHITE, TFT_NAVY);
  tft.setTextSize(1);
  tft.setCursor(4, 6);
  tft.printf("Catch: %d", catchCount);

  tft.setCursor(80, 6);
  tft.printf("Mode: %s", currentMode == MODE_AUTO ? "AUTO" : "STATIC");

  if (currentMode == MODE_STATIC) {
    tft.setCursor(160, 6);
    tft.printf("Col:%d", staticColorIdx);
    
    tft.setCursor(240, 6);
    tft.printf("Hz:%d", frequencies[staticFreqIdx]);
  }
  
  // Highlight active menu
  if (currentMenu != MENU_NONE) {
    tft.drawRect(0, 0, 320, 20, TFT_YELLOW); 
    // Indication of what is selected
    int x_underline = 0;
    int w_underline = 40;
    
    if (currentMenu == MENU_CATCH) x_underline = 4;
    else if (currentMenu == MENU_MODE) x_underline = 80;
    else if (currentMenu == MENU_COLOR) x_underline = 160;
    else if (currentMenu == MENU_FREQ) x_underline = 240;
    
    tft.fillRect(x_underline, 16, w_underline, 2, TFT_YELLOW);
  }
}
...
void decodeAndDisplay() {
  ...
  // Increment catch count on new frame received (assumed successful capture)
  catchCount++;

  tft.fillScreen(TFT_BLACK);
  // Offset Y by 20 for UI Bar
  TJpgDec.drawJpg(0, 20, jpegBuf, jpegSize);
